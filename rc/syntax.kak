# Originally taken from Coqoune: https://github.com/Guest0x0/coqoune/blob/32685fb65a8e5133037d53586d9e26cbcef3b13a/rc/syntax.kak
# but modified to fit my needs.
#
# This provides a better code highlighting than the default one.
...
# ------
    ...
    # numeral literals
    add-highlighter shared/coq/command/ regex \b[-]?[0-9][0-9_]*(\.[0-9_]+)?([eE][+-][0-9_]+)? 0:value
    ...
    evaluate-commands %sh{
        ...
        commands="${commands} Lemma Let Library Load LoadPath Local Locate Ltac Module No Notation Opaque"
        commands="${commands} Parameter Parameters Primitive Print Proof Property Proposition Pwd Qed Quit"
        commands="${commands} Rec Record Redirect Register Remark Remove Require Reset Reserved"
        commands="${commands} Section Search SearchAbout SearchHead SearchPattern SearchRewrite Show Strategy"
        commands="${commands} Tactic Test Theorem Time Timeout Transparent Types Universes Undo Unfocus Unfocused Unset Unshelve Variable Variables"
        ...
        tactics="${tactics} compute congr congruence constructor contradict contradiction"
        tactics="${tactics} cut cutrewrite cycle decide decompose dependent"
        tactics="${tactics} destruct discriminate do done double"
        tactics="${tactics} eapply eassert eauto eexact eexists elim elimtype evar exact exfalso"
        ...
        tactics="${tactics} inversion_clear lapply lazy last left move omega"
        tactics="${tactics} pattern pose progress red refine reflexivity revert"
        ...
        printf %s "
            add-highlighter shared/coq/command/ regex \b(${commands_regex})\b 0:function
        "
